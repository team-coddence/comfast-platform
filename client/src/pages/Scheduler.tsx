...
const Scheduler = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || error.message || "Impossible de charger les messages");
    }
  }
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || error.message || "Échec de la planification");
    }finally{
      setLoading(false);
    }
  }
      ...
      <div className="w-full lg:w-[460px] shrink-0">
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <div className="flex items-center gap-2 mb-6">
              <h2 className="text-lg text-slate-700">Composer un message</h2>
            </div>

            <form className="space-y-5" onSubmit={handleSchedule}>
              {/* Platforms */}
              <div>
                <label className="block text-xs text-slate-500 uppercase mb-2">Plateformes</label>
              ...
              {/* Content */}
              <div>
                <label className="block text-xs text-slate-500 uppercase mb-2">Contenu</label>
                <textarea required rows={5} placeholder="Que souhaitez-vous partager aujourd'hui ?" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content} onChange={(e)=>setContent(e.target.value)}/>
              ...
              {/* Media upload */}
              <div>
                <label className="block text-xs text-slate-500 uppercase mb-2">Média (optionnel)</label>
                {mediaFile ? (
                  <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                    ...
                    <button type="button" onClick={()=> setMediaFile(null)} className="absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full flex items-center justify-center transition-colors cursor-pointer">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-slate-500 uppercase mb-2">Heure</label>
                  <div className="relative">
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
                ...
                <h3 className="text-slate-900 text-sm">À venir</h3>
              ...
              <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
                {scheduled.length === 0 ? (
                  <div className="py-10 text-center text-slate-400 text-sm">Aucun message planifié pour le moment</div>
                ) : (
                  scheduled.map((post)=>(
                    <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">{post.mediaType === "image" ? "image" : "vidéo"}</span>}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
                ...
                <h3 className="text-slate-900 text-sm">Publiés</h3>
                <span className="ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">{published.length}</span>
              </div>
              <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
                {published.length === 0 ? (
                  <div className="py-10 text-center text-slate-400 text-sm">Aucun message publié pour le moment</div>
                ) : (
                  published.map((post)=>(
                    <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-100 px-2 py-0.5 rounded-full">Publié</span>
                          </div>
                      </div>
