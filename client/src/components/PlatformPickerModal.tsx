            ...
            <div className="p-6 flex flex-col gap-2">
                {PLATFORMS.map((p)=>{
                    ...
                    return (
                        ...
                        className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-primary-200 bg-primary-50 cursor-default" : "border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}>

                            {/* Icon */}
                            <div className="p-2">
                                <p.icon className={`size-5 ${isConnected ? "text-primary-600" : "text-slate-500"}`}/>
                            </div>

                            {/* Label */}
                            <div className="flex-1 min-w-0">
                                <div className={`text-sm ${isConnected ? "text-primary-700" : "text-slate-800"}`}>
                                    {p.name}
                                </div>
                              ...
                              {/* Status */}
                            {isConnected && <CheckCircleIcon className="size-4 text-primary-500 shrink-0"/>}
                            {isConnecting && <div className="size-4 border-2 border-primary-600 border-t-transparent rounded-full animate-spin shrink-0"/>}
