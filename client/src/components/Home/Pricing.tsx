            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                <div className="text-center mb-16">
                    ...
                    <h2 className="font-serif font-medium text-4xl sm:text-5xl leading-tight text-gray-900">
                        ...
                        <br />
                        <span className="text-primary-500 italic">de croissance</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                    {pricingPlans.map((plan) => (
                        <div key={plan.name} className={`rounded-2xl border p-7 flex flex-col gap-6 relative ${plan.highlight ? "bg-primary-500 text-white border-primary-400 shadow-2xl shadow-primary-100" : "bg-white text-slate-900 border-slate-200"}`}>
                            ...
                            <div>
                                <div className={`text-sm font-semibold mb-1 ${plan.highlight ? "text-primary-100" : "text-primary-500"}`}>{plan.name}</div>
                                <div className="flex items-end gap-1">
                                    <span className="text-4xl font-bold">{plan.price}</span>
                                    <span className={`text-sm mb-1.5 ${plan.highlight ? "text-primary-200" : "text-slate-400"}`}>{plan.period}</span>
                                </div>
                                <p className={`text-sm mt-2 leading-relaxed ${plan.highlight ? "text-primary-100" : "text-slate-500"}`}>{plan.description}</p>
                            </div>
 
                            <ul className="space-y-2.5">
                                {plan.features.map((f) => (
                                    <li key={f} className="flex items-center gap-2.5 text-sm">
                                        <div className={`size-4 rounded-full flex items-center justify-center shrink-0 ${plan.highlight ? "bg-primary-400" : "bg-primary-50"}`}>
                                            <CheckIcon className={`w-2.5 h-2.5 ${plan.highlight ? "text-white" : "text-primary-500"}`} />
                                        </div>
                                        <span className={plan.highlight ? "text-primary-50" : "text-slate-600"}>{f}</span>
                                    </li>
                                ))}
                            </ul>
 
                            <Link to="/#" className={`mt-auto text-center font-semibold text-sm px-6 py-3 rounded-full cursor-pointer ${plan.highlight ? "bg-white text-primary-500 hover:bg-primary-50" : "bg-primary-500 text-white hover:bg-primary-600"}`}>
                                {plan.cta}
                            </Link>
                        </div>
                    ))}
